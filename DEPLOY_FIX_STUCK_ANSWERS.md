# Deploy: Fix Stuck Answers + Draft Flicker

Copy the SQL below into the **Supabase SQL Editor** and run it. This rewrites two RPCs.

```sql
-- ============================================================================
-- Fix: Games stuck in "Waiting for players..." even when all answered
-- ============================================================================
-- Root cause: submit_arena_answer and force_close_question filter "active"
-- players by last_seen > NOW() - 15s. If a player's heartbeat lags (network
-- blip, tab backgrounded, etc.), they're excluded from the count, causing
-- the all-answered transition to never fire.
-- ============================================================================

-- 1. Fix submit_arena_answer: count ALL players, remove last_seen filter
...
BEGIN
    ...
    -- F. Check ALL Players Answered (count ALL lobby players, not just recent heartbeat)
    ...
    IF v_total_player_count > 0 AND v_answer_count >= v_total_player_count THEN
        UPDATE lobbies
    ...
    RETURN json_build_object(
        ...
        'all_answered', (v_answer_count >= v_total_player_count),
        'total_players', v_total_player_count,
        'answers_received', v_answer_count
    );
END;
$$;

-- 2. Fix force_close_question: timeout all lobby players (not just heartbeat-active)
DROP FUNCTION IF EXISTS force_close_question(text);
...
BEGIN
    ...
    -- FIX: Count ALL players in lobby (not just recent heartbeat)
    FOR v_player IN 
        SELECT p.id, p.name 
        FROM players p
...
1. Open [Supabase Dashboard](https://supabase.com/dashboard/project/xurxuikgxnrzmrgkwkhy) → SQL Editor
2. Paste the SQL above into a new query
3. Click **Run**
4. You should see: `Success. No rows returned.`

The frontend fixes (draft flicker, client-side `all_answered` optimistic transition) are already committed locally — push to GitHub triggers Vercel auto-deploy.

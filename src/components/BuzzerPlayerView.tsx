...
import { Zap, Users, Trophy, Wifi, WifiOff, ArrowRight, LogIn, Circle } from "lucide-react";
...
export default function BuzzerPlayerView() {
  ...
  const { broadcast, onBroadcast, presences, isConnected } = useRealtimeChannel({
    ...
    onLobbyChange: (payload: any) => {
      ...
      if (newLobby.status) {
        ...
        // Reset buzz state when host opens buzzers again
    ...
    onPlayerChange: async () => {
      ...
      if (data) {
        setPlayers(data.sort((a: any, b: any) => b.score - a.score));
        // Update own score
        if (playerId) {
  ...
  useEffect(() => {
    ...
    // Verify player still exists in lobby
    supabase
      .from("players")
      ...
      .then(({ data, error }) => {
        if (error || !data) {
          // Player doesn't exist anymore — go back to join
  ...
  const handleJoin = useCallback(async () => {
    ...
    try {
      ...
      const { data: lobby, error: lobbyErr } = await supabase
        .from("lobbies")
        .select("code, status, buzzed_player_id")
        .eq("code", code)
        .single();
  ...
  const handleBuzz = useCallback(async () => {
    ...
    // Success! Keep success state until host clears
      ...
      <div className="shrink-0 px-4 py-3 flex items-center justify-between border-b border-warm-gray/10 bg-warm-white/80 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          {phase === "PLAY" && (
            <button
              onClick={handleLeave}
              className="text-[10px] font-bold text-warm-gray/40 hover:text-peach transition-colors"
            >
              Leave
            </button>
          )}
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-sm space-y-8 animate-clay-pop">
            ...
            <div className="text-center space-y-2">
              ...
              <h1 className="text-4xl font-outfit font-black text-plum tracking-[0.15em]">
                {code}
              </h1>
              <p className="text-sm text-warm-gray/60 font-medium">
                Enter your name to join the buzzer game
              </p>
            </div>
            ...
            <div className="space-y-4">
              <input
                ...
                maxLength={20}
                autoFocus
                className="w-full text-center font-outfit font-black text-2xl text-plum placeholder:text-warm-gray/30
                  px-6 py-4 rounded-2xl bg-warm-white border-2 border-warm-gray/20
                  focus:border-soft-purple/40 focus:outline-none focus:ring-4 focus:ring-soft-purple/10
                  transition-all uppercase tracking-wide"
              />

              {joinError && (
                <div className="text-center text-xs font-bold text-peach bg-peach-light/50 px-4 py-2 rounded-xl">
                  {joinError}
                </div>
              )}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl
                  bg-soft-purple text-white font-outfit font-black text-lg
                  hover:bg-soft-purple/90 active:scale-[0.98] transition-all
                  disabled:opacity-40 disabled:active:scale-100
                  shadow-[4px_4px_0px_rgba(166,157,145,0.3)]
                  active:shadow-[2px_2px_0px_rgba(166,157,145,0.2)]"
              >
                {isJoining ? (
      ...
      {phase === "PLAY" && (
        <div className="flex-1 flex flex-col p-4 sm:p-6 max-w-lg mx-auto w-full gap-6">
          ...
          <div className="flex justify-center">
            <div
              className={`inline-flex items-center gap-2 px-5 py-2 rounded-full border-2 font-outfit font-black text-sm uppercase tracking-wider transition-all duration-300 ${getStatusColor(
                gameStatus
              )}`}
            >
              <Circle
                className={`w-2 h-2 fill-current ${
                  gameStatus === "BUZZING" ? "animate-pulse" : ""
                }`}
              />
          ...
          {buzzState === "buzzed-success" && (
            <div className="text-center space-y-2 animate-clay-pop">
              <p className="font-outfit font-black text-mint text-lg">
                🎉 You buzzed in!
              </p>
              <p className="text-xs text-warm-gray/60 font-medium">
                Wait for the host to grade your answer
              </p>
            </div>
          )}

          {buzzState === "buzzed-too-slow" && buzzedPlayerId && (
            <div className="text-center space-y-1">
              <p className="font-outfit font-bold text-peach text-sm">
                {players.find((p) => p.id === buzzedPlayerId)?.name || "Someone"} was faster!
              </p>
              <p className="text-[10px] text-warm-gray/40 font-medium">
                Wait for the next buzzer round
              </p>
            </div>
          )}
          ...
          <div className="flex items-center justify-center gap-4 px-6 py-4 bg-warm-white rounded-2xl border border-warm-gray/10 shadow-sm">
            <div className="text-center">
              <div className="text-[10px] font-black text-warm-gray/40 uppercase tracking-wider">
                Score
              </div>
              <div className="font-mono font-black text-3xl text-plum">
                {playerScore}
              </div>
            </div>
            {myRank > 0 && (
              <div className="w-px h-10 bg-warm-gray/10" />
            )}
            {myRank > 0 && (
              <div className="text-center">
                <div className="text-[10px] font-black text-warm-gray/40 uppercase tracking-wider">
                ...
                {players.map((p, i) => (
                  <div
                    ...
                    className={`flex items-center justify-between px-3 py-2 rounded-xl transition-all ${
                      p.id === playerId
                        ? "bg-soft-purple-light border border-soft-purple/20"
                        : "bg-warm-white border border-warm-gray/5"
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-outfit font-bold text-sm text-plum truncate max-w-[120px]">
                        {p.name}
                        {p.id === playerId && (
                          <span className="ml-1 text-[10px] text-soft-purple">(you)</span>
                        )}
                      </span>
                    </div>
                    <span className="font-mono font-bold text-sm text-plum">
                      {p.score || 0}
                    </span>
                  </div>
                ))}
                {players.length === 0 && (
                  <div className="text-center py-4 text-warm-gray/30 text-xs">
                    No players yet
                  </div>
                )}
              </div>

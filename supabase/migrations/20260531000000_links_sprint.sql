-- ============================================================================
-- LINKS SPRINT — Multiplayer Wave-Based Word Sprint
-- ============================================================================
-- 2-6 players type words containing shared letters. Computer assigns letters.
-- 5 hidden target words per wave with escalating difficulty bonuses.
-- Pure scoring — no hearts, no elimination. 2-3 waves.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.links_sprint_words (
    ...
    target_level INTEGER,  -- 1-5 if is_target is true
    wave INTEGER NOT NULL DEFAULT 1,
    created_at TIMESTAMPTZ DEFAULT NOW(),

    -- One claim per word per lobby per wave
...
BEGIN
    ...
    v_letter_count := v_player_count;  -- Same number of letters as players

    -- Generate initial letters (server picks random common letters)
    -- Client will refine with actual dictionary intersection
    v_letters := ARRAY['A','E','R','T','N','S','L','C','O','I','U','D','P','M','H','G','B','F','Y','W','K','V','X','Z','J','Q'];
    -- Just set a default; client overrides with weighted random
...
BEGIN
    ...
    FOR v_target_entry IN SELECT jsonb_array_elements(v_targets)
    LOOP
...
BEGIN
    ...
    -- Store target reveals for end-game display
    UPDATE lobbies
    SET arena_state = COALESCE(arena_state, '{}'::jsonb) || jsonb_build_object(
        'phase', 'WAVE_RESULTS',
        'targetReveals', COALESCE(arena_state->'targetReveals', '[]'::jsonb) || jsonb_build_object(
            'wave', v_current_wave,
            'targets', v_targets
        ),
        'scores', (
...
BEGIN
    ...
    -- Accumulate cumulative shuffle deductions per player
    v_update_json := jsonb_build_object(
        'shuffleDeductions', COALESCE(v_arena_state->'shuffleDeductions', '{}'::jsonb) ||
            jsonb_build_object(
                p_player_id::text,
                COALESCE((v_arena_state->'shuffleDeductions'->>(p_player_id::text))::integer, 0) + v_points_deduction
            ),

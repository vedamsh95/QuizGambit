...
--
-- Fix: Count ALL players in the lobby, not just recently active ones.
-- force_close_question already handles the case where a player never answers
-- (timer expires → timeout penalties). The server timer provides a safety net.
-- ============================================================================
...
BEGIN
    ...
    -- If a player genuinely disconnected, that's OK — they still get a timeout
    -- and the game transitions. A truly left player should have been removed.
    FOR v_player IN 
        SELECT p.id, p.name 
        FROM players p
